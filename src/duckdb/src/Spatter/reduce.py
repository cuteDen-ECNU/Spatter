...
def main():
    origin1 = '''
SELECT COUNT(*)         FROM tableA As a1          RIGHT OUTER JOIN tableA As a2 ON ST_Overlaps(a1.geom, a2.geom)          WHERE  a1.valid = True and a2.valid = True and a1.id <> a2.id;
    '''
    origin2 = '''
SELECT COUNT(*)         FROM tableB As a1          RIGHT OUTER JOIN tableB As a2 ON ST_Overlaps(a1.geom, a2.geom)          WHERE  a1.valid = True and a2.valid = True and a1.id <> a2.id;
    '''

    table1 = re.findall(r'\bFROM ([A-Za-z_]+) As\b', origin1)[0]
    table2 = re.findall(r'\bFROM ([A-Za-z_]+) As\b', origin2)[0]
    print(table1)
    print(table2)

    create_table1 = f'''CREATE TABLE {table1} As SELECT * FROM read_csv('/log/{table1}.csv', delim=',', header=true, ''' + '''columns={'id': 'Integer', 'geom': 'geometry', 'valid': 'BOOLEAN', 'gRelate': 'VARCHAR'});'''
    create_table2 = f'''CREATE TABLE {table2} As SELECT * FROM read_csv('/log/{table2}.csv', delim=',', header=true, ''' + '''columns={'id': 'Integer', 'geom': 'geometry', 'valid': 'BOOLEAN', 'gRelate': 'VARCHAR'});'''
    executor.execute(create_table1)
    executor.execute(create_table2)
